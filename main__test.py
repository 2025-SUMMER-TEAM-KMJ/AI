# main__test.py
# 이렇게 하면  추천 공고의 전문을 확인할 수 있음
#  (청크 인덱싱 + where_minimal 사용 + 점수기반 질의; 반환은 job_id 리스트)
...
def print_full_for_ids(job_ids: List[str], *, max_chars: int | None = None):
    
    for i, sid in enumerate(job_ids, 1):
        # (idx, doc, meta) -> stitch_chunks가 요구하는 (idx, doc)로 변환
        pairs = _get_all_chunks(sid)
        texts = [(idx, doc) for idx, doc, _ in pairs]
        full_text = stitch_chunks(texts, overlap_chars=OVERLAP_CHARS) if texts else ""

        if max_chars is not None:
            full_text = full_text[:max(0, int(max_chars))]

        print(f"[{i}] job_id={sid}")
        print(full_text)
        print("\n" + "-" * 100 + "\n")
...
if __name__ == "__main__":
    ...
    print_full_for_ids(result_job_ids, max_chars=None)
